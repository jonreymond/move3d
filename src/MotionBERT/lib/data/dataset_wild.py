...
def coco17_to_h36m17(x):
    y = np.zeros((x.shape[0], 17, 3))  # assume x: (N, 17, 3)
    ...
    y[:, 8] = x[:, 0]                   # Thorax approximated by nose
    y[:, 9] = x[:, 0]                   # Neck/Nose (same as nose here)
    y[:,10] = x[:, 0]                   # Head (same as nose)
    y[:,11] = x[:, 5]                   # LShoulder
    y[:,12] = x[:, 7]                   # LElbow
    y[:,13] = x[:, 9]                   # LWrist
    y[:,14] = x[:, 6]                   # RShoulder
...
def read_input(json_path, vid_size, scale_range, focus):
    ...
    kpts_all = coco17_to_h36m17(kpts_all)
    if vid_size:
        w, h = vid_size
        scale = min(w,h) / 2.0
